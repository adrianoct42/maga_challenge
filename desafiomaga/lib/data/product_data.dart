import "package:desafiomaga/model/product.dart";

var products = [
  Product("Amoeba", 10, "assets/Amoeba.jpg",
      "Ganhar isso de presente de natal é uma lembrança que eu nunca perdi com o tempo, naquele presente embrulhado da árvore de natal. Mas esse produto tinha um cheiro tão insuportável com uso prolongado que minha mãe jogou fora sem eu saber! >:("),
  Product("MP3", 200, "assets/MP3.jpg",
      "Depois de sequestrar um desses do meu irmão mais velho, foi simplesmente o aparelho que me fez gostar de música. Ouvindo o que moleque anos 2000 ouvia de praxe: Foo Fighters, Linkin Park, System of a Down, Green Day e outros. Muita memória boa!"),
  Product("Kit de Ping Pong", 40, "assets/PingPong.jpg",
      "Quando estava no ensino fundamental, colocaram uma mesa de ardósia de Ping Pong no intervalo e aquilo era simplesmente a coisa mais próxima de um MMA de adolescente numa escola particular do interior de MG. Competição acirrada!"),
  Product("Pipa de Morcego", 30, "assets/PipaMorcego.jpg",
      "Meu primo tinha uma dessas e a gente se divertia demais no condomínio dele. Essa praga voava tão longe e tão bem que advinha? Uma hora voou pro além mesmo e só sobrou nosso choro."),
  Product("Super Nintendo", 400, "assets/SNES.jpg",
      "Até hoje é meu console favorito. Lotado de coisa boa e jogos que simplesmente o mercado indie atual tenta replicar com algum esplendor, a tal era dos pixels 16bits. As vezes eles conseguem. Eram tempos simples e muito felizes (meu Deus eu tô muito nostálgico)."),
  Product("Castlevania: Symphony of The Night", 50, "assets/SOTN.jpg",
      "Um dos jogos mais importantes da minha vida. Me introduziu ao gênero -tipo metroid- e causa boa impressão até em quem nunca se aventurou por ele nos dias de hoje. Uma combinação supimpa de exploração, visual, música e fator replay. Symphony of The Night é uma pérola atemporal."),
];
